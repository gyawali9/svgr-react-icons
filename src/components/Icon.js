...
const Icon = ({ src, size, fill = "currentColor", className, style, isUrl = false, alt, ...rest }) => {
        ...
        if (isUrl && typeof src === "string") {
            ...
            fetch(src)
                ...
                if (fill && fill !== "currentColor") {
                    modifiedData = modifiedData.replace(/fill="[^"]*"/g, `fill="${fill}"`);
                }
    ...
    if (isUrl && svgContent) {
        return (_jsx("svg", { ref: svgRef, className: className, style: svgStyle, dangerouslySetInnerHTML: { __html: svgContent }, role: alt ? "img" : undefined, "aria-label": alt, ...rest }));
    }
    if (typeof src === "function") {
        // SVGR-generated component
        const SvgComponent = src;
        return (_jsx(SvgComponent, { className: className, style: svgStyle, role: alt ? "img" : undefined, "aria-label": alt, ...rest }));
    }
